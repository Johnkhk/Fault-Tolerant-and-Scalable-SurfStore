...
func (s *RaftSurfstore) attemptCommit(targetIdx int64, commitIdx int) {
	commitChan := make(chan *AppendEntryOutput, len(s.raftAddrs))
	for idx := range s.raftAddrs {
		if int64(idx) == s.serverId {
			continue
		}

		go s.commit_Entry(int64(idx), targetIdx, commitChan)
	}
	...
	for {
		...
		if commCount > len(s.raftAddrs)/2 {
...
func (s *RaftSurfstore) commit_Entry(serverIdx, entryIdx int64, commitChan chan *AppendEntryOutput) {
	for {
		addr := s.raftAddrs[serverIdx]
